...
def build_final_messages(
    ...
    *,
    ...
    if len(converted_links) == 1:
        return [
            _join_header_and_section(
                broadcast_header,
                "\n".join(["📥 Download Link", "", _escape_html_text(converted_links[0])]),
            )
        ]

    chunks: list[list[tuple[int, str]]] = []
    current: list[tuple[int, str]] = []
    for index, link in enumerate(converted_links, start=1):
        candidate = [*current, (index, link)]
        if (
            current
            and len(_numbered_message(broadcast_header, "📥 Download Links", candidate))
            > max_length
        ):
    ...
    for part_number, chunk in enumerate(chunks, start=1):
        ...
        if total_parts > 1:
            heading = f"{heading} {part_number}/{total_parts}"
        messages.append(_numbered_message(broadcast_header, heading, chunk))
    return messages
