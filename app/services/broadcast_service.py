...
async def deliver_job(
    ...
    *,
    ...
    while True:
        ...
        status, error = await _send_parts(bot, subscriber_id, parts)
        async with session_factory() as session:
    ...
    for part in parts:
        try:
            ...
            try:
                ...
                return DeliveryStatus.BLOCKED, type(retry_forbidden).__name__
            except TelegramError as retry_error:
                return DeliveryStatus.FAILED, type(retry_error).__name__
        except Forbidden as exc:
            return DeliveryStatus.BLOCKED, type(exc).__name__
        except TelegramError as exc:
            return DeliveryStatus.FAILED, type(exc).__name__
